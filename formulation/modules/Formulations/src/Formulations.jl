...
function formulation2(inst::InstanceData)
	model = defineModel()
	...
	@variable(model, 0 <= bc[i=1:inst.NOP] <= BigM, Int)
	@variable(model, 0 <= fc[i=1:inst.NOP] <= BigM, Int)
	@variable(model, 0 <= c[m=1:inst.NM, t=1:inst.NOP] <= 10, Int)
	@variable(model, 0 <= fmax <= BigM, Int)
	...
	@constraint(model, beginAfterFinishCutTime[i=2:inst.NOP, l=1:inst.NOP, m=1:inst.NM; l<i], bc[i] >= fc[l] - BigM*(2 - x[i,m] - x[l,m]))
